...
def insert_record(filename: str, matter_no: str, date_due: str, folder_path: str):
    with sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        cur.execute(
            """
            INSERT INTO matters (filename, matter_no, date_due, folder_path, created_at)
            VALUES (?, ?, ?, ?, ?)
            """,
            (filename, matter_no, date_due, folder_path, datetime.utcnow().isoformat(timespec="seconds"))
        )
        conn.commit()
...
def fetch_all_records():
    with sqlite3.connect(DB_PATH) as conn:
        ...
        cur.execute(
            """
            SELECT filename, matter_no, date_due, folder_path
            FROM matters
...
def export_to_excel():
    ...
    headers = ["Filename", "Matter No.", "Date_Due", "Link to folder"]
    for col, header in enumerate(headers, start=1):
        ws.cell(row=1, column=col, value=header)

    for r_idx, row in enumerate(rows, start=2):
        filename, matter_no, date_due, folder_path = row
...
#Apply Bold Font to header row
    ...
    for cell in ws[1]:   #access the first row in Header Row
        cell.font= bold_font
...
class App(tk.Tk):
    ...
    def save_record(self):
        ...
        try:
            insert_record(filename, matter_no, date_due, str(folder_path))
        except Exception as e:
